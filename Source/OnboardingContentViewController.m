...
#define DEFAULT_BORDER_RADIUS 0.0;
#define DEFAULT_BUTTON_BACKGROUND_COLOR [UIColor clearColor];
#define DEFAULT_BUTTON_PADDING_WIDTH 0.0;
#define DEFAULT_BUTTON_HEIGHT 50.0;
...
- (instancetype)initWithTitle:(NSString *)title body:(NSString *)body image:(UIImage *)image videoURL:(NSURL *)videoURL buttonText:(NSString *)buttonText actionBlock:(action_callback)actionBlock {
    ...
    self.buttonPaddingWidth = DEFAULT_BUTTON_PADDING_WIDTH;
    self.buttonHeight = DEFAULT_BUTTON_HEIGHT;
    self.buttonBorderRadius = DEFAULT_BORDER_RADIUS;
    self.buttonBackgroundColor = DEFAULT_BUTTON_BACKGROUND_COLOR;
...
- (void)generateView {
    ...
    if (_buttonText) {
        _actionButton = [[UIButton alloc] initWithFrame:CGRectMake((CGRectGetMaxX(self.view.frame) / 2) - ((contentWidth-self.buttonPaddingWidth) / 2), CGRectGetMaxY(self.view.frame) - self.underPageControlPadding - kMainPageControlHeight - self.buttonHeight - self.bottomPadding, contentWidth-self.buttonPaddingWidth, self.buttonHeight)];
        ...
        _actionButton.layer.cornerRadius = self.buttonBorderRadius;
